				   ...
				   unsigned long cr4_pinned_mask)
{
	...
	wrmsrl(MSR_KVM_CR0_PINNED_LOW, ~cr0_pinned_bits & cr0_pinned_mask & mask);
	wrmsrl(MSR_KVM_CR0_PINNED_HIGH, cr0_pinned_bits & cr0_pinned_mask & mask);

	rdmsrl(MSR_KVM_CR4_PIN_ALLOWED, mask);
	wrmsrl(MSR_KVM_CR4_PINNED_LOW, ~cr4_pinned_bits & cr4_pinned_mask & mask);
	wrmsrl(MSR_KVM_CR4_PINNED_HIGH, cr4_pinned_bits & cr4_pinned_mask & mask);
}
