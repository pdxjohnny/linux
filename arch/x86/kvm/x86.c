...
static int __set_sregs(struct kvm_vcpu *vcpu, struct kvm_sregs *sregs)
{
	...
	vcpu->arch.harden.cr0_pinning &= sregs->cr0;
	kvm_x86_ops->set_cr0_guest_owned_bits(vcpu,
			vcpu->arch.cr0_guest_owned_bits & ~vcpu->arch.harden.cr0_pinning);

	mmu_reset_needed |= kvm_read_cr0(vcpu) != sregs->cr0;
	kvm_x86_ops->set_cr0(vcpu, sregs->cr0);
	vcpu->arch.cr0 = sregs->cr0;

	vcpu->arch.harden.cr4_pinning &= sregs->cr4;
	kvm_x86_ops->set_cr4_guest_owned_bits(vcpu,
			vcpu->arch.cr4_guest_owned_bits & ~vcpu->arch.harden.cr4_pinning);
