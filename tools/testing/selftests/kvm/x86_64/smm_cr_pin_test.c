...
#define CR4_PINNED_HIGH (X86_CR4_SMAP | X86_CR4_UMIP)
#define CR4_PINNED_LOW (X86_CR4_SMEP | X86_CR4_FSGSBASE)
#define CR4_ALL (CR4_PINNED_HIGH | CR4_PINNED_LOW)
...
void guest_code(void)
{
	...
	set_cr4(get_cr4() | CR4_PINNED_HIGH);

	sync_with_host(4);

	wrmsr(MSR_KVM_CR4_PINNED_HIGH, CR4_PINNED_HIGH);
	wrmsr(MSR_KVM_CR4_PINNED_LOW, CR4_PINNED_LOW);
	...
	for (failure = 0; failure <= 1; failure++) {
		...
		for (stage = 1;; stage++) {
			...
			if (stage_reported == DONE) {
				...
				TEST_ASSERT((sregs.cr4 & CR4_ALL) == CR4_PINNED_HIGH,
			...
			if (stage_reported == SMRAM_STAGE_FAILURE) {
				...
				*cr &= ~CR0_PINNED;
				...
				/* Unset pinned high, set pinned low */
				*cr &= ~CR4_PINNED_HIGH;
				*cr |= CR4_PINNED_LOW;
			}
